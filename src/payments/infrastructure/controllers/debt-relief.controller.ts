...
export class DebtReliefController {
   ...
   createDebtRelief = async (req: Request, res: Response) => {
      try {
         ...
         const debtReliefProps = new DebtRelief({
            ...createDebtReliefDTO,
         ...
         return this.sendResponse(res, 500, null, error as Error);
      }
   };
   ...
   private sendResponse(res: Response, statusCode: number, responseData: {code: string, message: string, data?: DebtRelief[] | DebtRelief} | null, error: Error | null) {
      if (error) {
         responseData = {
            code: '99',
